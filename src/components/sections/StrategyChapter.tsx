"use client";

import Reveal from "@/components/motion/Reveal";
import { chapters, images } from "@/lib/data";

const strategy = chapters.strategy;

export default function StrategyChapter() {
  return (
    <section id="strategy" className="relative">
      <div className="lg:flex">
        {/* Sticky image — left */}
        <div className="relative h-[50vh] lg:sticky lg:top-0 lg:h-screen lg:w-[55%]">
          <img
            src={images.strategy}
            alt=""
            className="chapter-image"
            loading="lazy"
          />
          <div className="image-overlay image-overlay-bottom lg:hidden" />
          <div className="hidden lg:block image-edge-right" />
          <div className="image-overlay" style={{ background: "rgba(5,5,7,0.2)" }} />
        </div>

        {/* Scrolling content — right */}
        <div className="relative z-10 lg:w-[45%]">
          {/* Panel 1: Title */}
          <div className="flex min-h-[60vh] items-center px-6 py-20 lg:min-h-screen lg:px-14">
            <Reveal>
              <div>
                <p className="text-[13px] uppercase tracking-[0.2em] text-accent">
                  {strategy.eyebrow}
                </p>
                <h2 className="mt-4 text-4xl font-semibold tracking-tight text-foreground lg:text-5xl">
                  {strategy.title}
                </h2>
                <p className="mt-6 max-w-lg text-lg leading-relaxed text-muted-foreground">
                  {strategy.description}
                </p>
              </div>
            </Reveal>
          </div>

          {/* Layer panels — each layer scrolls as its own mini-screen */}
          {strategy.layers.map((layer, i) => (
            <div
              key={layer.name}
              className="flex min-h-[50vh] items-center px-6 py-12 lg:min-h-[60vh] lg:px-14"
            >
              <Reveal delay={0.05}>
                <div className="flex gap-6">
                  <span className="shrink-0 text-5xl font-bold leading-none tracking-tight text-accent/25 lg:text-6xl">
                    0{i + 1}
                  </span>
                  <div>
                    <h3 className="text-2xl font-semibold tracking-tight text-foreground lg:text-3xl">
                      {layer.name}
                    </h3>
                    <p className="mt-3 max-w-sm text-[15px] leading-relaxed text-muted-foreground">
                      {layer.desc}
                    </p>
                  </div>
                </div>
              </Reveal>
            </div>
          ))}

          {/* Final panel: Skills */}
          <div className="flex min-h-[50vh] items-center px-6 py-16 lg:min-h-[60vh] lg:px-14">
            <Reveal delay={0.05}>
              <div>
                <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">
                  Applied through
                </p>
                <div className="mt-6 flex flex-wrap gap-3">
                  {strategy.skills.map((skill) => (
                    <span
                      key={skill}
                      className="rounded-full border border-border bg-surface px-4 py-2 text-sm text-foreground transition-colors hover:border-accent/40"
                    >
                      {skill}
                    </span>
                  ))}
                </div>
              </div>
            </Reveal>
          </div>
        </div>
      </div>
    </section>
  );
}
